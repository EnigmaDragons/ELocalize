...
namespace I2.Loc
{
	public partial class LocalizationEditor
	{
		...
		{
			Debug.Log("Begun Checking For Total Scene Localization Progress");
			...
			for (var i = 0; i < length; i++)
			{
				...
				Debug.Log($"{SceneManager.GetActiveScene().name,-32} - Localization Progress: {percentComplete:P} - Done: {done} - Needed: {objs.Count}");
			...
			Debug.Log($"Overall - Localization Progress: {finalPercentComplete:P} - Done: {finalDone} - Needed: {finalNeeded}");
		...
		static void PrintTop5GameObjectNames(List<string> objNames)
		{
			objNames
				...
				.Take(5)
				.ToList()
				.ForEach(a => Debug.Log($"Object: {a.Key} - {a.Count()}"));
		}
		...
		static void SelectNoLocalizedLabels()
		{
			...
			if (objs.Count > 0)
			{
				...
				Debug.Log($"{SceneManager.GetActiveScene().name} - Localization Progress: {percentComplete:P} - Done: {localizedCount} - Needed: {objs.Count}");
